...
import { useSolicitud } from "../hooks/useSolicitud"; // Agregar esta importación
...
const HistorialDetallesPage: React.FC = () => {
  ...
  const { aprobarSolicitud } = useSolicitud(); // Agregar esta línea
  ...
  const [updatingSolicitudes, setUpdatingSolicitudes] = useState<{
    [key: string]: boolean;
  }>({}); // Agregar estado para solicitudes
  const [solicitudEstados, setSolicitudEstados] = useState<{
    [key: string]: string;
  }>({}); // Estado local para los estados de solicitud
  ...
  const handleUpdateDetalleEstado = async (
    detalleId: number,
    accion: string,
    item: any
  ) => {
    ...
    try {
      ...
      if (accion === "aprobar") {
        ...
        toast.success(`Detalle #${detalleId} prestado exitosamente`);
      ...
      toast.error(
        `Error al ${accion} el detalle: ${error instanceof Error ? error.message : "Error desconocido"}`
      );
    } finally {
    ...
    return buttons.map((button) => {
      ...
      return (
        <button
          key={button.accion}
          onClick={() =>
            handleUpdateDetalleEstado(detalleId, button.accion, item)
          }
          disabled={isUpdating}
          className={`
  ...
  // Agregar función para manejar cambio de estado de solicitud
  const handleUpdateSolicitudEstado = async (solicitudId: number, item: any) => {
    if (!user?.usuario.id) {
      toast.error("No se pudo identificar el usuario");
      return;
    }
  
    const updateKey = `solicitud-${solicitudId}`;
    setUpdatingSolicitudes((prev) => ({ ...prev, [updateKey]: true }));
  
    try {
      await aprobarSolicitud(solicitudId, user.usuario.id);
      
      // ❌ ELIMINAR: Estas líneas causan el doble refresco
      // if (selectedPersonaId) {
      //   await fetchHistorialPorPersona(selectedPersonaId);
      // } else {
      //   await fetchHistorialCompleto();
      ...
      setSolicitudEstados((prev) => ({ 
        ...prev, 
        [solicitudId]: 'APROBADA' 
      }));
      
      toast.success("Solicitud aprobada exitosamente");
    } catch (error) {
      toast.error(
        `Error al aprobar la solicitud: ${error instanceof Error ? error.message : "Error desconocido"}`
      );
    } finally {
      setUpdatingSolicitudes((prev) => ({ ...prev, [updateKey]: false }));
    }
  };

  // Función para obtener el estado actual de la solicitud
  const getCurrentSolicitudEstado = (item: any) => {
    return solicitudEstados[item.solicitudId] || item.solicitudEstado;
  };
        ...
        {historial.length === 0 ? (
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-500">
              No se encontró historial de detalles
            </p>
          </div>
        ) : (
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="border-t pt-3">
                    ...
                    {item.fichaNumero && (
                      <p className="text-sm text-gray-900">
                        Ficha:{" "}
                        <span className="font-medium">{item.fichaNumero}</span>
                      </p>
                    )}
                    {item.tituladoNombre && (
                      <p className="text-sm text-gray-900">
                ...
                {(item.centroNombre || item.sedeNombre) && (
                  <div className="border-t pt-3">
                    ...
                    {item.centroNombre && (
                      <p className="text-sm text-gray-900">
                        Centro:{" "}
                        <span className="font-medium">{item.centroNombre}</span>
                      </p>
                    )}
                    {item.sedeNombre && (
                      <p className="text-sm text-gray-900">
                        Sede:{" "}
                        <span className="font-medium">{item.sedeNombre}</span>
                      </p>
                    )}
                    {item.sedeDireccion && (
                      <p className="text-xs text-gray-500">
                ...
                {item.sitioDestinoNombre && (
                  <div className="border-t pt-3">
                    ...
                    {item.tipoSitioNombre && (
                      <p className="text-xs text-gray-500">
                        Tipo: {item.tipoSitioNombre}
                      </p>
                    )}
                  </div>
                )}
                ...
                {item.estado && (
                  <div className="border-t pt-3">
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">
                      Estado de tu petición
                    </p>
                    <p className="text-sm text-gray-700">
                      Tu petición se encuentra: {item.estado}
                    </p>
                  </div>
                )}
                ...
                <div className="border-t pt-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {item.fechaActualizacion &&
                      item.fechaActualizacion !== item.fechaSolicitud && (
                        <p className="text-xs text-gray-500">
