...
export default function AcercaDePage() {
  const { user } = useAuth();
  const [personaCompleta, setPersonaCompleta] =
    useState<PersonaCompleta | null>(null);
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300">
        <div className="flex items-center mb-3">
          <div
            className={`p-2 rounded-lg ${colorClasses[color as keyof typeof colorClasses] || colorClasses.green}`}
          >
            <Icon className="h-5 w-5" />
          </div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 ml-3">
            {title}
          </h3>
        </div>
  ...
  const StatCard = ({
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`p-3 rounded-full ${color.includes("green") ? "bg-green-100 dark:bg-green-900/20" : "bg-green-100 dark:bg-green-900/20"}`}
        >
  ...
  const renderTabContent = () => {
    switch (activeTab) {
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <InfoCard
              icon={Activity}
              title="Estado"
              value={personaCompleta.personaActiva ? "Activo" : "Inactivo"}
              color={personaCompleta.personaActiva ? "green" : "red"}
            />
            <InfoCard
              icon={Clock}
              title="Última Sesión"
              value="Hoy"
              subtitle="Conectado actualmente"
              color="teal"
            />
          </div>
        );
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                    {personaCompleta.identificacion}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 dark:text-gray-400">
      ...
      case "about":
        return (
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-green-600 to-purple-600 rounded-lg shadow-lg p-8 text-white">
              <div className="flex items-center mb-4">
                <Code className="h-8 w-8 mr-3" />
                <h3 className="text-2xl font-bold">Sistema SABS</h3>
              </div>
              <p className="text-green-100 mb-4">
                Sistema de Administración de Bienes y Servicios - Una solución
                integral para la gestión de inventarios, movimientos y reportes
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 mb-8 border border-gray-200 dark:border-gray-700">
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
            <div className="bg-gradient-to-r from-green-500 to-purple-600 rounded-full p-4">
